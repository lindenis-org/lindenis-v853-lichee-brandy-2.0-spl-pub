...
void main(void)
{
	...
	extern void boot_e907(uint32_t base);
	if (cpus_rtos_base != 0)
		boot_e907(cpus_rtos_base);
#endif
...
#error only spinor support this feature
#endif

int spinor_read(uint, uint, void *);
int load_kernel_from_spinor(u32 *kernel_addr)
{
	int ret = 0;
#ifdef CFG_KERNEL_BOOTIMAGE
	u32 kernel_start = CFG_SPINOR_KERNEL_OFFSET + CFG_SPINOR_LOGICAL_OFFSET;
	ret = load_bimage(kernel_start, CFG_KERNEL_LOAD_ADDR, spinor_read,
			  kernel_addr);
	if (ret < 0) {
		printf("load bootimage erro\n");
//CFG_SPINOR_KERNEL_BACKUP_OFFSET=0x37A0
#ifdef CFG_SPINOR_KERNEL_BACKUP_OFFSET
		u32 kernel_backup = CFG_SPINOR_KERNEL_BACKUP_OFFSET +
				    CFG_SPINOR_LOGICAL_OFFSET;
...
#define UIMAGE_SIZE_KB (2 * 1024)
	ret = load_uimage(CFG_SPINOR_KERNEL_OFFSET + CFG_SPINOR_LOGICAL_OFFSET,
...
void load_and_run_kernel(u32 optee_base, u32 opensbi_base)
{
	...
	load_flash_success = load_kernel_from_spinor(&kernel_addr);
#endif
